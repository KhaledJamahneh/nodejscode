...
const createDeliveryRequest = async (req, res) => {
  try {
    ...
    const request = await transaction(async (client) => {
      // 1. Get client profile ID and active status (LOCK ROW)
      const clientResult = await client.query(
        `SELECT cp.id, cp.remaining_coupons, cp.subscription_type, cp.subscription_expiry_date, cp.current_debt, u.is_active
      ...
      // 3. Check if subscription type matches payment method
      if (payment_method === 'coupon_book' && clientData.subscription_type !== 'coupon_book') {
        const error = new Error('Cash subscription clients cannot use coupon payment. Please change your payment method to cash.');
        error.status = 400;
        throw error;
      }

      // 4. Check if debt exceeds limit (₪10,000) for cash requests
      if ((payment_method === 'cash' || clientData.subscription_type === 'cash') && parseFloat(clientData.current_debt) > 10000) {
        const error = new Error('Credit limit exceeded. Please pay your outstanding balance of ₪' + clientData.current_debt + ' to continue.');
        error.status = 403;
        throw error;
      }

      // 5. Check if subscription has expired (with 10-day grace period)
      if (clientData.subscription_expiry_date) {
        ...
        gracePeriodDate.setDate(gracePeriodDate.getDate() + 10); // 10 days grace
        
        const today = new Date();
        if (today > gracePeriodDate) {
          const error = new Error('Your subscription has expired. Please renew to continue making requests.');
          error.status = 403;
          throw error;
        }
      }

      // 6. Check if client has enough coupons (if using coupon book)
      ...
      if (parseInt(pendingRequests.rows[0].count) >= 3) {
        const error = new Error('You already have 3 pending requests. Please wait for them to be processed.');
        error.status = 400;
        throw error;
      }
      ...
      const result = await client.query(
        ...
        [clientData.id, priority || 'non_urgent', requested_gallons, payment_method || 'cash', notes, 'pending']
      );
